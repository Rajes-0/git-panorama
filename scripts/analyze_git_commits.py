...
class GitCommitsAnalyzer:
    def __init__(self, config_file: str):
        ...
        self.exclusion_patterns = self.compile_patterns(
            self.config['exclusions']['patterns']
        )
        self.inclusion_patterns = self.compile_patterns(
            self.config['exclusions']['always_include']
        )
    ...
    def load_cached_repo_data(self, repo_path: Path) -> Optional[List[dict]]:
    ...
    def should_exclude_file(self, file_path: str, repo_name: str) -> bool:
        ...
        for pattern, _ in self.exclusion_patterns:
            if pattern.match(file_path):
                return True
    ...
    def parse_git_log_line(self, line: str) -> Optional[dict]:
    ...
    def generate_bulk_data(self, commits_data: List[dict], index_name: str) -> str:
        ...
        for commit in commits_data:
            ...
            action = {
                "index": {
                    "_index": index_name,
                    "_id": f"{commit['repository']}_{commit['commit_id']}"
                }
            }
    ...
    def analyze_all_repositories(self):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_repo = {executor.submit(self.analyze_repository, repo_path): repo_path 
    ...
    def generate_summary(self, commits_data: List[dict]):
        ...
        summary = {
            ...
            'repositories': len(set(c['repository'] for c in commits_data)),
            'contributors': len(set(c['author_name'] for c in commits_data))
        }
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: analyze_git_commits.py <config_file>")
        sys.exit(1)
    
    config_file = sys.argv[1]
    analyzer = GitCommitsAnalyzer(config_file)
